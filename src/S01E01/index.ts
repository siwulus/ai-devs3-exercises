...
const url = process.env.XYZ_AGENTS_URL ?? '';
const username = process.env.XYZ_AGENTS_USERNAME ?? '';
const password = process.env.XYZ_AGENTS_PASSWORD ?? '';
...
const sendForm = (answer: string): TaskEither<Error, string> => {
  ...
  return pipe(
    ...
    chain(html =>
      tryExecute('parse form response')(async () => {
        const { document } = parseHTML(html);
...
await pipe(fetchQuestion(), chain(findAnswer), chain(sendForm), toPromise);
